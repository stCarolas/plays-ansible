...
idx_selected=$(printf '%s\n' "${windows_separators[@]}" | rofi $ROFI_OPTIONS -dmenu -i -p "$command_" -a "$index_workspace_active" -format i -selected-row "$index_window_last_active" -no-custom -s -width 80 -lines 30 -markup-rows)
